...
[TestFixture]
public class BasicCalculatorTask2Tests
{
	private BasicCalculatorTask2 _task = null!;

	[SetUp]
	public void SetUp() => _task = new BasicCalculatorTask2();
	...
	[Test]
	public void Calculate_ParenthesesWithAddition_ReturnsCorrectResult()
	{
		_task.Calculate("(2 + 3) + 4").Should().Be(9);
	}
	...
	{
		_task.Calculate("5 + (3 + (4 - 1))").Should().Be(11);
	}
	...
	[Test]
	public void Calculate_UnaryMinusAfterSubtraction_ReturnsCorrectResult()
	{
	...
	[Test]
	public void Calculate_ParenthesesWithSubtraction_ReturnsCorrectResult()
	{
		_task.Calculate("10 - (3 + 4) - 5").Should().Be(-2);
	}

	[Test]
	public void Calculate_DeeplyNestedParentheses_ReturnsCorrectResult()
	{
		_task.Calculate("(((1 + 2) + 3) - 4) - 5").Should().Be(-3);
	}
	...
	[Test]
	public void Calculate_OnlyAdditionAndSubtraction_ReturnsCorrectResult()
	{
		_task.Calculate("10 + 2 - 6 - 4 + 2").Should().Be(4);
	}
}
